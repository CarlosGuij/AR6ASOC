...
foreach ($data as $estacion) {
    ...
    $latitud = floatval($coordenadas[0]);

    if ($latitud > 0) {
        $estacionesNorte++;
    } else {
        $temperatura = intval($estacion['numberrange']);
        if ($temperatura < -20 || $temperatura > 40) {
            $estacionesExtremas[] = $estacion['estacion'];
        }
        if ($latitud < 0) {
            $temperaturasSur[] = $temperatura;
        }
    }
}

$temperaturaMediaSur = count($temperaturasSur) > 0 ? array_sum($temperaturasSur) / count($temperaturasSur) : 0;

echo "Estaciones en el Hemisferio Norte: $estacionesNorte\n";
"<br>";
echo "Temperatura media en el Hemisferio Sur: $temperaturaMediaSur\n";
"<br>";
echo "Estaciones con temperaturas extremas: " . implode(', ', $estacionesExtremas) . "\n";
"<br>";
